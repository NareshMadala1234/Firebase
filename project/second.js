    ...
    if (user) {
      ...
      firebase.database().ref('blogs/').orderByChild('userid').equalTo(uid).on("value",function(snapshot){
        ...
        for(let[key,value] of Object.entries(myData)){
            ...
            "<button class='btn btn-danger' id= '"+key+"' style='background-color: 	#ff0000;color: black;font-weight:2px;font-style:Times-roman;border: 2px solid black;' onclick = 'deletePost(this.id)'>Delete</button>"+
        ...
        console.log("Upload is" + progress + "done");
                ...
                firebase.database().ref('blogs/').push().set({
                        ...
                        if(error){
                            ...
                            alert("Successfully uploaded the pic");
...
function getData(){
    firebase.database().ref('blogs/').once('value').then(function(snapshot){
        ...
        for(let[key,value] of Object.entries(data)){
            posts_div.innerHTML = "<div class='col-sm-4 mt-2 mb-1'>"+
            "<div class = 'card'>" +
            "<img src= '"+value.imageURL+"' style='display: block;margin-left: auto;margin-right: auto;font-weight:bold;height:250px;width:100%'>" +
